...
var liveProps = map[xml.Name]struct {
	...
	findFn func(context.Context, FileSystem, LockSystem, string, module.FileNode) (string, error)
...
func props(ctx context.Context, fs FileSystem, ls LockSystem, name string, pnames []xml.Name, fi module.FileNode) ([]Propstat, error) {
...
func propnames(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) ([]xml.Name, error) {
	/*f, err := fs.OpenFile(ctx, name, os.O_RDONLY, 0)
	if err != nil {
		return nil, err
	}
...
func allprop(ctx context.Context, fs FileSystem, ls LockSystem, name string, include []xml.Name, fi module.FileNode) ([]Propstat, error) {
...
func findResourceType(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) (string, error) {
	if fi.IsFolder {
...
func findDisplayName(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) (string, error) {
	if slashClean(name) == "/" {
		// Hide the real name of a possibly prefixed root directory.
		return "", nil
	}
	return escapeXML(fi.FileName), nil
}

func findContentLength(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) (string, error) {
	return strconv.FormatInt(fi.FileSize, 10), nil
}

const LOGTIMEFORMAT = "2006-01-02 15:04:05"

func findLastModified(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) (string, error) {
	location, _ := time.LoadLocation("Local")
	localTime, _ := time.ParseInLocation(LOGTIMEFORMAT, fi.LastOpTime, location)
	return localTime.UTC().Format(http.TimeFormat), nil
}

func findCreate(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) (string, error) {
	location, _ := time.LoadLocation("Local")
	localTime, _ := time.ParseInLocation(LOGTIMEFORMAT, fi.CreateTime, location)
	return localTime.UTC().Format(http.TimeFormat), nil
}
...
func findContentType(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) (string, error) {
	return "", nil
...
func findETag(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) (string, error) {
	...
	time, _ := time.Parse("2006-01-02 15:04:05", fi.LastOpTime)
	return fmt.Sprintf(`"%x%x"`, time.UnixNano(), fi.FileSize), nil
}

func findSupportedLock(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi module.FileNode) (string, error) {
	return `` +
